...
当前 WebUI 已提供格式刷、批量选择和替换工具。

### 11.1 格式刷

格式刷适合快速复制某个已设计单元格的属性组合。使用方式是：

- 先选中一个 `designed` 单元格作为源格
- 勾选要刷入的字段：地形、生态、标签、备注
- 点击格式刷按钮进入格式刷模式
- 点击目标单元格应用所选字段
- 再次点击可退出格式刷模式

默认只刷地形与生态；标签和备注需要手动勾选。

### 11.2 批量选择

顶栏或右侧高级编辑面板中的 `批量` / `批量选择` 会进入批量选择模式。
...
- 拖拽画布仍然用于平移，不会误选
- 右侧高级编辑面板会显示已选数量
- 点击 `应用到选中格` 后，会用面板中的地形、生态、标签和备注批量设置这些单元格

批量设置会生成一次本地编辑历史；仍需点击顶栏 `保存` 才会写入地图文件。

### 11.3 替换工具

高级编辑面板还提供：

- `替换地形`：把全图中某种 terrain 替换为另一种 terrain
- `替换生态`：把全图中某种 biome 替换为另一种 biome，也可匹配或替换为无生态

替换会经过 terrain / biome 组合校验。如果替换后的组合不合法，界面会显示错误信息，地图不会被改写。
...
- 浏览器会直接触发下载
- 默认导出当前加载区域，适合大地图；也可以切换为全图导出
- 服务端会拒绝超大地图的全图 PNG 导出，请改用区域导出
