...
在当前版本已经完成 WebUI / CLI 基础闭环并通过首轮手工验收后，下一阶段不再以“从零搭骨架”为重点，而转向“修正契约、提升体验、收紧规则、补齐说明”。

下一阶段建议拆分为 5 个连续 Phase，按顺序推进。

### 18.1 Phase A：契约修正

目标：

- 让当前实现重新对齐设计案中的接口与存储约定
- 在继续补功能前先修正底层偏差，减少后续返工

范围：

- 修正 CLI `maps apply --stdin` / `--file` 输入契约
- 对齐默认存储目录到仓库根目录 `storage/maps` 与 `storage/exports`
- 保留必要的兼容策略，避免破坏当前已可用流程

建议实现点：

- `maps apply` 支持设计案中的 `{ "commands": [...] }` 包装结构
- 为避免破坏现有可用性，建议兼容：
  - 单条 `MapCommand`
  - `MapCommand[]`
  - `{ "commands": MapCommand[] }`
- server 默认读写路径统一回归到仓库根目录 `storage/...`
- 继续保留 `MAPDESIGNER_ROOT` 覆盖能力，便于测试和临时隔离环境
...
### 18.2 Phase B：图片导出下载体验

目标：

- 让 WebUI 的图片导出从“服务端落文件”升级为“用户直接得到浏览器下载”

范围：

- WebUI 导出图片后触发浏览器下载
- 保留 server / CLI 的文件落盘能力
- 提升导出成功 / 失败反馈

建议实现点：

- server 提供可供 WebUI 下载的图片访问方式
- WebUI 在点击“导出图片”后可直接触发下载
- 保留当前导出目录写文件能力，确保 CLI 与调试流程不受影响
- 优先保证 `clean / reference` 预设行为不变

完成判定：

- WebUI 点击“导出图片”后浏览器能直接下载 PNG
- CLI `export-png` 继续可用
- 导出失败时能得到清晰反馈

### 18.3 Phase C：terrain 选择结构升级

目标：

- 降低地形选择的扁平长列表负担，提升 WebUI 可操作性

范围：

- 为 `terrain` 增加一级分类结构
- WebUI 由单层选择改为“一级分类 -> 具体地形”两级选择
- 保持地图文件格式和 CLI 数据键不变

建议实现点：

- 基于当前地形字典进一步整理人类可读的一级分类
- WebUI 编辑时先选分类，再选该分类下的具体地形
- 打开已有地图时，要能正确回填分类与具体地形
- 只在 UI 层和字典组织方式上升级，不改变持久化数据结构

完成判定：

- Terrain 不再以单个超长扁平列表呈现
- 用户可以通过分类更快定位具体地形
- 已有地图打开后地形值能稳定回填

### 18.4 Phase D：terrain / biome 匹配规则增强

目标：

- 减少明显不合理组合，提升地图设定的现实感和一致性

范围：

- 梳理并增强 `terrain / biome` 校验规则
- 明确哪些组合应视为 `invalid`
- 保留一部分创作弹性，允许边缘情况以 `warning` 存在

建议实现点：

- 优先禁止明显违和组合，例如海洋类地形搭配陆地森林生态
- 保持 CLI、WebUI、导入校验共用同一套规则
- 第一阶段先把保存校验做稳，不急着一次做很重的联动筛选
- 如后续需要，再补 biome 选项按 terrain 过滤的增强交互

完成判定：

- 明显不合理组合会被稳定拦截
- 合理组合不受影响
- CLI 与 WebUI 对同一组合给出一致结论

### 18.5 Phase E：验收与文档收口

目标：

- 在下一阶段主要功能补齐后，把导入导出、CLI 契约和使用说明完整收口

范围：

- 完成 JSON 导入流程的手工验收与必要测试
- 完成 JSON round-trip 验收
- 更新 README 和操作说明
- 补充 CLI 输入格式、文件位置、图片导出下载说明

建议实现点：

- 增补导入冲突、非法 tag、非法组合、保存冲突等验收与测试
- 在 README 中明确：
  - 如何启动 WebUI / server
  - 地图文件保存位置
  - CLI 基本命令
...
1. Phase A：契约修正
2. Phase B：图片导出下载体验
3. Phase C：terrain 选择结构升级
4. Phase D：terrain / biome 匹配规则增强
5. Phase E：验收与文档收口

排序理由：

- 先修契约，再做体验，可避免后续在偏差基础上继续扩展
- 图片下载是高感知收益项，适合在契约修正后优先落地
- terrain 分类与 terrain/biome 匹配规则彼此关联，适合相邻阶段推进
- 文档与验收最适合在主要改动稳定后统一补齐

### 18.7 当前优先建议

如果只启动下一阶段的第一包工作，建议先执行：

1. CLI `maps apply` 输入契约对齐
2. 默认存储目录对齐
3. 相关测试与文档同步更新

完成这一步后，再进入图片下载和地形/生态选择升级，会更稳妥。
